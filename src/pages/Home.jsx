...
const Home = () => {
  ...
  const banners = [
  {img:reliableImage, heading :'Reliable',text:'we have devoted our time to giving you the best experience possible'},
  {img:timeImage,heading:'Save Time',text:'Don’t waste time sending tokens one at a time let us handle it'},
  {img:speedImage,heading:'Speed',text:'we have devoted our time to giving you the best experience possible'}
...
<Header/>
    ...
    md:w-1/2
     ...
     items-center 
     text-center">
      <h1 className="title-font sm:text-4xl text-3xl mb-4 font-medium text-red-600">Welcome</h1>
      <p className="mb-8 leading-relaxed">We are here to make crypto transactions easier</p>
     

     <p className='text-2xl w-10/12'>Send crypto to multiple
     addresses with <span className='text-orange-600'>
      ...
      <div className="flex justify-center my-8">
        <Link to={'/send-token'}>
        <button className=" bg-orange-600 border-0 
        py-3 px-10 rounded-3xl text-lg font-medium" >Send Tokens</button>
        </Link>
           </div>
    </div>
    <div className="
   ...
   {banners.map((item)=>(
    ...
    sm:my-0
     ...
     text-center">
      ...
      <Card className="sm:w-72  sm:h-80 px-4 bg-gray-50  h-auto pb-14 pt-5 rounded-3xl">
...
<div className='flex justify-center mx-auto w-32 h-32' >
  ...
  <div >
    ...
    <h3 className='text-center font-medium pb-4 pt-2'>{item.heading}</h3>
  </div>
